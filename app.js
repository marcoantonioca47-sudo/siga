...
function details(){return '<div class="page-title"><div><h1>Detalhes das Operações</h1><p>Todos os dados disponíveis de cada operação.</p></div></div><div class="panel"><div class="detail-grid">'+[...rows('separacoes').map(function(x){return Object.assign({_tipo:'Separação'},x)}),...rows('carregamentos').map(function(x){return Object.assign({_tipo:'Carregamento'},x)}),...rows('conferencias').map(function(x){return Object.assign({_tipo:'Conferência'},x)}),...rows('atividades').map(function(x){return Object.assign({_tipo:'Atividade'},x)})].map(function(r){return '<div class="detail-card"><div><span class="detail-type">'+esc(r._tipo)+'</span>'+badge(r.resultado||r.status)+'</div><h3>'+esc(r.lote||r.romaneio||r.referencia||r.pedido||'Operação')+'</h3><div class="detail-fields">'+Object.keys(r).filter(function(k){return k!==\'_tipo\'&&r[k]!==undefined&&r[k]!==\'\'}).map(function(k){return '<div class="detail-field"><span>'+esc(k.replace(/_/g,' '))+'</span><b>'+esc(r[k])+'</b></div>'}).join('')+'</div></div>'}).join('')+'</div></div>';}\nfunction userDisplayName(id){
 const u=accountList().find(function(x){return norm(x.id)===norm(id)||norm(x.name)===norm(id);});
 return u&&u.name?u.name:(id||'Não informado');
}
function occurrences(){
 const list=[...rows('conferencias').filter(function(x){return /não ok|nao ok/i.test(x.resultado||'')||x.ocorrencia&&x.ocorrencia!=='—'}).map(function(x){return Object.assign({_tipo:'Conferência'},x)}),...rows('carregamentos').filter(function(x){return /não ok|nao ok/i.test(x.resultado||'')||x.ocorrencia}).map(function(x){return Object.assign({_tipo:'Carregamento'},x)}),...rows('separacoes').filter(function(x){return /não ok|nao ok/i.test(x.resultado||'')||x.ocorrencia}).map(function(x){return Object.assign({_tipo:'Separação'},x)})];
 return '<div class="page-title"><div><h1>Ocorrências</h1><p>Todos os detalhes da operação relacionada à ocorrência.</p></div></div><div class="panel"><div class="occ-list">'+(list.length?list.map(function(r){
 const uid=r.usuario_id||r.user_id||r.usuario||r.operador_id||r.operador||r.separador_id||r.separador;
 const nomeSeparador=userDisplayName(uid);
 return '<div class="occ-card"><div class="occ-icon">⚠</div><div><b>'+esc(r.referencia||r.lote||r.romaneio||r.pedido||'Ocorrência')+'</b><div class="detail-fields"><div class="detail-field"><span>SEPARADOR RESPONSÁVEL</span><b>'+esc(nomeSeparador)+'</b></div>'+Object.keys(r).filter(function(k){return k!=='_tipo'&&k!=='usuario_id'&&k!=='user_id'&&k!=='usuario'&&k!=='operador_id'&&k!=='operador'&&k!=='separador_id'&&k!=='separador'&&r[k]!==undefined&&r[k]!==''}).map(function(k){return '<div class="detail-field"><span>'+esc(k.replace(/_/g,' '))+'</span><b>'+esc(r[k])+'</b></div>'}).join('')+'</div></div>'+badge(r.resultado||r.status||'NÃO OK')+'</div>'
 }).join(''):'<div class="empty">Nenhuma ocorrência encontrada.</div>')+'</div></div>';
}
